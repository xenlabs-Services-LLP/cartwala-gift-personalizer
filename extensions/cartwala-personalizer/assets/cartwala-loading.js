(()=>{
  ...
  let overlay=null,hideTimer=0,progressTimer=0,watchdog=0,progress=0,shownAt=0,observer=null,cartCompleted=false;
  ...
  const hide=()=>{clearTimeout(hideTimer);stopTimers();observer?.disconnect();observer=null;overlay?.classList.remove('is-visible');document.body.classList.remove('cw-processing-active','cw-cart-preparing');progress=0;shownAt=0;cartCompleted=false};
  ...
  const start=()=>{clearTimeout(hideTimer);stopTimers();observer?.disconnect();cartCompleted=false;shownAt=Date.now();const el=ensure();el.querySelector('.cw-processing-text').textContent='Adding your personalised item…';paint(5);el.classList.add('is-visible');document.body.classList.add('cw-processing-active','cw-cart-preparing');progressTimer=setInterval(()=>{if(progress>=94)return;const step=progress<35?5:progress<70?3:progress<88?2:1;paint(Math.min(94,progress+step))},220);observer=new MutationObserver(checkReady);observer.observe(document.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:['src','data-cw-preview','data-cw-draft-id']});checkReady();watchdog=setTimeout(()=>{console.warn('Cartwala cart preview readiness timed out; releasing cart after successful add');if(cartCompleted)complete();else hide()},15000)};
  const cartComplete=()=>{cartCompleted=true;if(checkReady())return;setTimeout(()=>{if(document.body.classList.contains('cw-cart-preparing'))complete()},900)};

  // Preview & Save stays instant. Progress belongs only to Add to Cart.
  // A successful cart add is authoritative: never hold the customer at 94% waiting for a theme-specific image marker.
  document.addEventListener('cartwala:cart-start',start);
  document.addEventListener('cartwala:cart-complete',cartComplete);
  document.addEventListener('cartwala:cart-error',hide);
  document.addEventListener('cartwala:compatibility-cart-add',cartComplete);
  document.addEventListener('cart:updated',()=>{if(cartCompleted)cartComplete();else checkReady()});
  document.addEventListener('product:added',()=>{if(cartCompleted)cartComplete();else checkReady()});
