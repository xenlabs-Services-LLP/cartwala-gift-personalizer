(()=>{
  ...
  let overlay=null,hideTimer=0,progressTimer=0,watchdog=0,completeTimer=0,progress=0,shownAt=0,cartConfirmed=false;
  ...
  const stopTimers=()=>{clearInterval(progressTimer);progressTimer=0;clearTimeout(watchdog);watchdog=0;clearTimeout(completeTimer);completeTimer=0};
  ...
  document.addEventListener('cartwala:cart-complete',()=>{cartConfirmed=true;paint(Math.max(progress,96));clearTimeout(completeTimer);completeTimer=setTimeout(complete,2500)});
  document.addEventListener('cartwala:item-added',()=>{cartConfirmed=true;paint(Math.max(progress,96));clearTimeout(completeTimer);completeTimer=setTimeout(complete,2500)});
