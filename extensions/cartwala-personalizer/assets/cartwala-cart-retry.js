(()=>{
  ...
  const decodeFile=file=>new Promise((resolve,reject)=>{
    const url=URL.createObjectURL(file);
    const image=new Image();
    const cleanup=()=>URL.revokeObjectURL(url);
    image.onload=()=>{cleanup();resolve(image)};
    image.onerror=()=>{cleanup();reject(new Error('Could not read personalization image'))};
    image.src=url;
  });
  ...
  const compressImage=async(file,key)=>{
    try{
      ...
      const isPreview=/Personalised\s+Preview/i.test(key||'');
      const maxSide=isPreview?1100:1600;
      ...
      const blob=await new Promise(resolve=>canvas.toBlob(resolve,'image/jpeg',isPreview?.78:.82));
      if(!blob||blob.size>=file.size)return file;
      const base=(file.name||'photo').replace(/\.[^.]+$/,'');
      return new File([blob],`${base}.jpg`,{type:'image/jpeg',lastModified:Date.now()});
    }catch(error){
      console.warn('Cartwala image compression skipped',error);
      return file;
    }
  };
  ...
  const optimizeForm=async source=>{
    ...
    for(const [key,value] of source.entries()){
      if(isImageFile(value))target.append(key,await compressImage(value,key));
  ...
  window.fetch=async(input,init)=>{
    ...
    const optimizedBody=await optimizeForm(init.body);
    const optimizedInit={...init,body:optimizedBody,headers:retryHeaders(init.headers)};
    let firstResponse=null;
    try{
      firstResponse=await nativeFetch(input,optimizedInit);
      if(firstResponse.ok){
        document.dispatchEvent(new CustomEvent('cartwala:cart-complete',{bubbles:true}));
        return firstResponse;
      }
    }catch(error){
      console.warn('Cartwala optimized cart request failed; trying compatibility mode',error);
    }
    ...
    try{
      ...
      document.dispatchEvent(new CustomEvent('cartwala:cart-error',{bubbles:true}));
      return fallback;
    }catch(error){
