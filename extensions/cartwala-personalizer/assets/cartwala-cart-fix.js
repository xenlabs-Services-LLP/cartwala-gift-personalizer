(()=>{
  ...
  const restorePreview=async()=>{const root=cartRoot();let designId='';try{designId=sessionStorage.getItem('cartwala-last-design')||''}catch{}if(!designId||designId===lastDesign)return;const list=rows(root);const row=list[list.length-1];if(!row)return;const image=row.querySelector('.cart-item__image,img');if(!image)return;const record=await getDraft(designId);if(!record?.blob)return;const url=URL.createObjectURL(record.blob);image.removeAttribute('srcset');image.removeAttribute('sizes');image.closest('picture')?.querySelectorAll('source').forEach(source=>{source.removeAttribute('srcset');source.removeAttribute('sizes')});image.src=url;image.dataset.cwDraftId=designId;image.style.objectFit='contain';lastDesign=designId};
  ...
  const openDrawer=drawer=>{document.body.classList.remove('cw-cart-preparing');if(typeof drawer?.open==='function'){drawer.open();return}drawer?.classList.add('active','animate');drawer?.setAttribute('open','');drawer?.setAttribute('aria-hidden','false');document.body.classList.add('overflow-hidden')};
  const sectionIds=drawer=>{
    const ids=[];const add=id=>{if(id&&!ids.includes(id))ids.push(id)};
    add(drawer?.dataset?.section);add(drawer?.getAttribute?.('data-section-id'));add(drawer?.closest?.('[id^="shopify-section-"]')?.id?.replace('shopify-section-',''));
    add('cart-drawer');add('cart-icon-bubble');return ids;
  };
  const refreshAndOpenCart=async()=>{
    ...
    lastDesign='';scheduleLight(0);schedulePreview(30);openDrawer(drawer);
    setTimeout(()=>{scheduleLight(0);schedulePreview(0)},180);
  };

  const observer=new MutationObserver(mutations=>{let cartChanged=false;for(const mutation of mutations){for(const node of mutation.addedNodes){if(!(node instanceof Element))continue;if(node.matches?.(`${drawerSelector},.cart-item,[data-cart-item],cart-drawer-item,.drawer__cart-item`)||node.querySelector?.('.cart-item,[data-cart-item],cart-drawer-item,.drawer__cart-item')){cartChanged=true;break}}if(cartChanged)break}scheduleLight(0);if(cartChanged)schedulePreview(40)});
