...
async function main() {
  ...
  const createProduct = async (design, model) => {
    ...
    const result = await gql(`mutation CreateProduct($product:ProductCreateInput!){productCreate(product:$product){product{id title handle tags variants(first:10){nodes{id}} media(first:10){nodes{id alt status}}} userErrors{field message}}}`, { product: {
      ...
      descriptionHtml: `<p>Create a meaningful romantic gift with the ${design.name} personalised ${model.label.toLowerCase()}. Upload ${design.photos === 1 ? "one favourite photo" : `${design.photos} favourite photos`} and add ${design.textLabel.toLowerCase()} to preview your mug before ordering.</p><ul><li>${design.photos} editable photo ${design.photos === 1 ? "area" : "areas"}</li><li>Editable ${design.textLabel.toLowerCase()}</li><li>High-quality full-wrap printing</li><li>Live preview before adding to cart</li><li>Carefully packed for safe delivery</li></ul><p>For the best print result, upload clear, high-resolution photos.</p>`,
      productType: "Customised Mug",
      vendor: "Cartwala",
      status: "ACTIVE",
      seo: { title: `${design.name} Personalised Love Mug | Cartwala`, description: `Create a ${design.name} personalised love mug with ${design.photos} photo${design.photos > 1 ? "s" : ""} and editable ${design.textLabel.toLowerCase()}. Preview online and order from Cartwala.` },
      tags: ["cw-mug", "cw-mug-love", model.modelTag, `cw-mug-template-${design.code.toLowerCase()}`, `cw-design-${design.code.toLowerCase()}`],
    } });
  ...
  for (const design of designs) {
    ...
    for (const row of designRows) {
      ...
      tags.push("cw-mug", "cw-mug-love", model.modelTag, `cw-mug-template-${design.code.toLowerCase()}`, `cw-design-${design.code.toLowerCase()}`);
      const updated = await gql(`mutation UpdateProduct($product:ProductUpdateInput!){productUpdate(product:$product){product{id title handle tags variants(first:10){nodes{id}} media(first:40){nodes{id alt status}}} userErrors{field message}}}`, { product: { id: product.id, title, tags: [...new Set(tags)], status: "ACTIVE", seo: { title: `${design.name} Personalised Love Mug | Cartwala`, description: `Create a ${design.name} personalised love mug with ${design.photos} photo${design.photos > 1 ? "s" : ""} and editable ${design.textLabel.toLowerCase()}. Preview online and order from Cartwala.` } } });
  ...
  console.log(`Love products ready: ${designs.length} designs × ${models.length} mug types = ${rows.length} products.`);
}

main().catch((error) => {
  console.error(`Love product upload failed: ${error.stack || error.message}`);
  process.exitCode = 1;
